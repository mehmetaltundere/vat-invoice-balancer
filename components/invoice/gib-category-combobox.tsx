...
import {
  OFFICIAL_GIB_VAT_CATEGORIES,
  GibVatCategory,
} from "@/lib/data/gib-categories";
import { Search, Info, Check, ChevronDown, Sparkles } from "lucide-react";

interface GibCategoryComboBoxProps {
  onSelectCategory: (category: GibVatCategory) => void;
}
...
export function GibCategoryComboBox({
  ...
  const filteredCategories = OFFICIAL_GIB_VAT_CATEGORIES.filter((cat) =>
    ...
    <div className="relative flex items-center gap-2" ref={containerRef}>
      ...
      <div className="relative flex-1">
        <div
          ...
          className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl bg-white border border-gray-200 text-xs font-semibold text-gray-800 hover:border-gray-300 cursor-pointer shadow-xs transition-all"
        >
          <div className="flex items-center gap-2 truncate">
            <Search className="h-3.5 w-3.5 text-gray-400 shrink-0" />
            <span className="truncate text-gray-700">
              {query || "Resmi GİB KDV Kategorisi Ara veya Seç..."}
            </span>
          </div>
          ...
          <div className="absolute left-0 top-full mt-1.5 w-full z-50 rounded-xl bg-white border border-gray-200 shadow-xl overflow-hidden animate-in fade-in-50 zoom-in-95">
            <div className="p-2 border-b border-gray-100 bg-gray-50 flex items-center gap-2">
              ...
              <input
                ...
                placeholder="Örn: Gözlük, Tekstil, Gıda..."
                autoFocus
            ...
            <div className="max-h-60 overflow-y-auto p-1.5 space-y-1">
              {filteredCategories.length === 0 ? (
                <div className="p-3 text-center text-xs text-gray-400 font-medium">
                  Eşleşen GİB kategorisi bulunamadı.
                </div>
              ) : (
                filteredCategories.map((cat) => (
                  <div
                    ...
                    className="p-2.5 rounded-lg hover:bg-gray-100 cursor-pointer transition-colors flex items-center justify-between"
                  >
                    <div>
      ...
      <div className="relative">
        <button
          ...
          className="p-2 rounded-xl text-gray-400 hover:text-[#0066CC] hover:bg-gray-100 transition-all cursor-pointer"
        >
          <Info className="h-4 w-4" />
        </button>

        {showTooltip && (
          <div className="absolute right-0 top-full mt-2 w-72 p-3 bg-gray-900 text-white text-[11px] leading-relaxed rounded-xl shadow-2xl z-50 border border-gray-700 animate-in fade-in-50">
